...
function numeroReciboDesdeSecuencia(seq) { return "RCS-" + String(Number(seq || 0) + 1).padStart(6, "0"); }
function numeroDeRecibo(recibo) { return Number(String(recibo || "").replace(/[^0-9]/g, "")) || 0; }
...
export default function App() {
  ...
  const [tickets, setTickets] = useState(() => load(TICKETS_KEY, []));
  const [reciboSeq, setReciboSeq] = useState(() => Number(load(RECIBO_SEQ_KEY, 0)) || 0);
  ...
  const [corrSocioId, setCorrSocioId] = useState("");
  ...
  const ticketsFiltrados = useMemo(() => { const q = norm(ticketQuery); return tickets.filter((t) => !q || norm(`${t.numeroRecibo || ""} ${t.socioNombre || ""} ${t.fecha || ""}`).includes(q)).sort((a,b)=>String(b.timestamp||"").localeCompare(String(a.timestamp||""))); }, [tickets, ticketQuery]);
  const corrSocio = (padron.socios || []).find((s) => String(s.id) === String(corrSocioId));
  const corrCuotas = corrSocio ? (padron.cuotas || []).filter((c) => String(c.socioId) === String(corrSocio.id)).sort((a,b)=>String(a.periodo||"").localeCompare(String(b.periodo||""))) : [];
  ...
  function cobrar() {
    ...
    const cob = {
      ...
      fecha: fechaCobro,
      numeroRecibo: numeroReciboDesdeSecuencia(reciboSeq),
      socioId: socio.id,
    ...
    const ticket = { ...cob, socio: { id: socio.id, numero: socio.numero || "", nombre: socio.nombre || "", dni: socio.dni || "", telefono: socio.telefono || "" }, cuotas: cuotasACobrar.map((c) => ({ ...c })) };
    setCobranzas((prev) => [cob, ...prev]);
    setTickets((prev) => [ticket, ...prev]);
    setReciboSeq((prev) => Math.max(Number(prev || 0), numeroDeRecibo(cob.numeroRecibo)));
...
¿Reemplazarlo?`)) return;
  ...
  // Exportación operativa: archivo liviano para importar las cobranzas móviles en CDT Gestión.
  ...
  // Copia integral: permite dejar la app exactamente con sus datos actuales luego de una actualización o cambio de equipo.
  function exportarBackupCompleto() {
    const backup = {
      version: "CDT_COBRADOR_BACKUP_COMPLETO_V1",
      appVersion: APP_VERSION,
      origen: "COBRADOR_CDT",
      fechaExportacion: new Date().toISOString(),
      datos: { padron, cobranzas, arqueos, tickets, reciboSeq, config }
    };
    downloadJson(backup, "CDT_backup_completo_" + fechaArchivo() + ".json");
  }
  ...
  function importarBackupCompleto(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Backup maestro nuevo.
        if (data.version === "CDT_COBRADOR_BACKUP_COMPLETO_V1" && data.datos) {
          const d = data.datos;
          if (!d.padron || !Array.isArray(d.padron.socios) || !Array.isArray(d.padron.cuotas) || !Array.isArray(d.cobranzas) || !Array.isArray(d.arqueos) || !Array.isArray(d.tickets)) {
            return alert("El backup completo no tiene una estructura válida.");
          }
          if (!confirm(`Restaurar copia completa?\n\nSocios: ${d.padron.socios.length}\nCuotas: ${d.padron.cuotas.length}\nCobranzas: ${d.cobranzas.length}\nArqueos: ${d.arqueos.length}\nEntradas/recibos: ${d.tickets.length}\n\nLos datos actuales de esta app serán reemplazados.`)) return;
          const maxHistorico = [...d.cobranzas, ...d.tickets].reduce((m, x) => Math.max(m, numeroDeRecibo(x.numeroRecibo)), 0);
          setPadron(d.padron);
          setCobranzas(d.cobranzas);
          setArqueos(d.arqueos);
          setTickets(d.tickets);
          setReciboSeq(Math.max(Number(d.reciboSeq || 0), maxHistorico));
          setConfig(d.config || config);
          setSocioId(""); setSeleccion([]); setBusqueda("");
          alert("Copia completa restaurada correctamente.");
          return;
        }

        // Compatibilidad con las exportaciones móviles anteriores: recupera cobranzas y arqueos sin tocar padrón/configuración.
        if (data.version === "CDT_COBRANZAS_MOVILES_V2" && Array.isArray(data.cobranzas)) {
          const arq = Array.isArray(data.arqueos) ? data.arqueos : [];
          if (!confirm(`Este archivo es una exportación de cobranzas para CDT Gestión.\n\nCobranzas: ${data.cobranzas.length}\nArqueos: ${arq.length}\n\n¿Recuperarlas también en esta app? El padrón y la configuración actuales se conservarán.`)) return;
          const existentes = new Map(cobranzas.map((c) => [String(c.idOperacion || c.numeroRecibo), c]));
          data.cobranzas.forEach((c) => existentes.set(String(c.idOperacion || c.numeroRecibo), c));
          const nuevasCobranzas = Array.from(existentes.values()).sort((a,b) => String(b.timestamp || b.fecha || "").localeCompare(String(a.timestamp || a.fecha || "")));
          const arqExistentes = new Map(arqueos.map((a) => [String(a.id || `${a.tipo || "ARQ"}-${a.fecha || ""}`), a]));
          arq.forEach((a) => arqExistentes.set(String(a.id || `${a.tipo || "ARQ"}-${a.fecha || ""}`), a));
          const maxHistorico = [...nuevasCobranzas, ...tickets].reduce((m, x) => Math.max(m, numeroDeRecibo(x.numeroRecibo)), 0);
          setCobranzas(nuevasCobranzas);
          setArqueos(Array.from(arqExistentes.values()));
          setReciboSeq(Math.max(reciboSeq, maxHistorico));
          alert(`Recuperación terminada. Cobranzas disponibles: ${nuevasCobranzas.length}. Próximo recibo: RCS-${String(Math.max(reciboSeq, maxHistorico) + 1).padStart(6, "0")}.`);
          return;
        }

        alert("El archivo seleccionado no es un backup completo ni una exportación de cobranzas compatible.");
      } catch {
  ...
  function reimprimirTicket(ticket) {
    const socioTicket = ticket.socio || { id: ticket.socioId, numero: ticket.socioNumero || "", nombre: ticket.socioNombre || "", dni: ticket.dni || "" };
    const cuotasTicket = Array.isArray(ticket.cuotas) && ticket.cuotas.length ? ticket.cuotas : (padron.cuotas || []).filter((c) => (ticket.cuotaIds || []).map(String).includes(String(c.id)));
    abrirTicket({ config, socio: socioTicket, cuotas: cuotasTicket, cobranza: ticket });
  }
  function cargarCorreccionCuota(id) {
    setCorrCuotaId(String(id || ""));
    const c = (padron.cuotas || []).find((x) => String(x.id) === String(id));
    setCorrConcepto(c?.concepto || "");
    setCorrImporte(c ? String(Number(c.importe || 0)) : "");
  }
  function guardarCorreccionCuota() {
    if (!corrCuotaId) return alert("Seleccioná una cuota.");
    const importe = Number(String(corrImporte || "").replace(/[^0-9]/g, ""));
    if (!importe || importe < 0) return alert("Ingresá un importe válido.");
    setPadron((prev) => ({ ...prev, cuotas: (prev.cuotas || []).map((c) => String(c.id) === String(corrCuotaId) ? { ...c, concepto: corrConcepto.trim() || "Cuota social", importe } : c) }));
  ...
  return <div className={`shell ${dark ? "dark" : ""}`}><style>{CSS}</style>
    ...
    <header className="top"><div className="brand"><div><div className="kicker">Club Deportivo Tacural</div><div className="title">Cobrador CDT</div></div><div className="status"><div><span className={`online-dot ${online ? "" : "offline"}`}></span>{online ? "En línea" : "Sin conexión"}</div><div>{config.cobrador || "Sin cobrador"}</div><span className="pill">{cobranzas.length} cobro/s · {money(totalCobrado)}</span><div className="app-version">Versión {APP_VERSION}</div></div></div><nav className="tabs">{[["cobrar","Cobrar"],["tickets","Entradas"],["arqueo","Arqueo"],["backup","Backup"],...(adminOk ? [["padron","Padrón"],["config","Config"]] : [])].map(([k,l])=><button key={k} onClick={()=>setTab(k)} className={`tab ${tab===k?"active":""}`}>{l}</button>)}</nav></header><main className="content stack">
    ...
    {tab === "config" && <><Card><h2 className="card-title">Configuración</h2><div className="grid two"><Field label="Club"><Input value={config.club} onChange={(e)=>setConfig({...config,club:e.target.value})} /></Field><Field label="Cobrador"><Input value={config.cobrador} onChange={(e)=>setConfig({...config,cobrador:e.target.value})} /></Field><Field label="Clave administrador"><Input type="password" value={config.claveAdmin || ''} onChange={(e)=>setConfig({...config,claveAdmin:e.target.value})} /></Field><Field label="Tema"><Select value={config.tema || 'oscuro'} onChange={(e)=>setConfig({...config,tema:e.target.value})}><option value="oscuro">Oscuro CDT</option><option value="claro">Claro</option></Select></Field><Field label="Medio por defecto"><Select value={config.medioDefault} onChange={(e)=>setConfig({...config,medioDefault:e.target.value,cuentaDefault:cuentasPorMedio(e.target.value)[0]})}><option>Efectivo</option><option>Transferencia</option><option>Mercado Pago</option><option>Débito</option></Select></Field><Field label="Cuenta por defecto"><Select value={cuentasPorMedio(config.medioDefault).includes(config.cuentaDefault)?config.cuentaDefault:cuentasPorMedio(config.medioDefault)[0]} onChange={(e)=>setConfig({...config,cuentaDefault:e.target.value})}>{cuentasPorMedio(config.medioDefault).map((c)=><option key={c}>{c}</option>)}</Select></Field><Field label="Comisión cobrador (%)"><Input type="number" min="0" step="0.01" value={config.comisionPorcentaje ?? 0} onChange={(e)=>setConfig({...config,comisionPorcentaje:e.target.value})} /></Field><Field label="Logo"><Input type="file" accept="image/*" onChange={cargarLogo} /></Field><Field label="Duplicado ticket"><Select value={config.imprimirDuplicado ? "Sí" : "No"} onChange={(e)=>setConfig({...config,imprimirDuplicado:e.target.value==="Sí"})}><option>Sí</option><option>No</option></Select></Field></div>{config.logo && <div style={{marginTop:14}} className="btn-row"><img src={config.logo} className="logo" alt="Logo" /><Button variant="red" onClick={()=>setConfig({...config,logo:""})}>Quitar logo</Button></div>}</Card><Card><h2 className="card-title">Corrección de cuotas importadas</h2><p className="muted">Uso exclusivo de Tesorería. La modificación reemplaza directamente la categoría y el importe de la cuota en este dispositivo.</p><div className="grid two"><Field label="Socio"><Select value={corrSocioId} onChange={(e)=>{setCorrSocioId(e.target.value);setCorrCuotaId("");setCorrConcepto("");setCorrImporte("")}}><option value="">Seleccionar socio</option>{(padron.socios||[]).slice().sort((a,b)=>String(a.nombre||"").localeCompare(String(b.nombre||""))).map((s)=><option key={s.id} value={s.id}>{s.numero} - {s.nombre}</option>)}</Select></Field><Field label="Cuota"><Select value={corrCuotaId} onChange={(e)=>cargarCorreccionCuota(e.target.value)} disabled={!corrSocioId}><option value="">Seleccionar cuota</option>{corrCuotas.map((c)=><option key={c.id} value={c.id}>{c.periodo} · {c.concepto || "Cuota social"} · {money(c.importe)}</option>)}</Select></Field><Field label="Categoría / concepto"><Input value={corrConcepto} onChange={(e)=>setCorrConcepto(e.target.value)} disabled={!corrCuotaId} /></Field><Field label="Valor de cuota"><Input type="text" inputMode="numeric" className="money-input" value={formatMoneyInput(corrImporte)} onChange={(e)=>setCorrImporte(parseMoneyInput(e.target.value))} disabled={!corrCuotaId} placeholder="$ 0" /></Field></div><div className="btn-row" style={{marginTop:12}}><Button variant="green" onClick={guardarCorreccionCuota} disabled={!corrCuotaId}>Guardar corrección</Button></div></Card><Card className="danger"><h2 className="card-title" style={{color:'#b91c1c'}}>Herramientas de producción</h2><p className="muted">Usalo una sola vez antes de comenzar la cobranza real. Borra datos de prueba y conserva la configuración general.</p><Button variant="red" onClick={limpiarTodo}>🗑 Reinicio inicial de producción</Button></Card></>}
  </main></div>;
}
